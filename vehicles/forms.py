...
class TripBeginForm(forms.Form):
    vehicle = forms.CharField(label='License Plate',
                              max_length=20,
                              required=True)
    ...
    odometer = forms.IntegerField(min_value=0,
                                  max_value=500000,
                                  required=True)
    is_fuel_full = forms.BooleanField(label='Fuel tank full',
                                      initial=False,
                                      required=False)
    were_tires_inspected = forms.BooleanField(label='Inspected tires',
                                              initial=False,
                                              required=False)
    completed_damage_inspection = forms.BooleanField(label='Checked for damage',
                                                     initial=False,
                                                     required=False)
...
class TripFinishForm(forms.Form):
    ...
    is_fuel_full = forms.BooleanField(label='Fuel tank full',
                                      initial=False,
                                      required=False)
    trash = forms.BooleanField(label='Free of trash',
                               initial=False,
                               required=False)
